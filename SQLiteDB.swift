...
class SQLiteDB {
	var DB_NAME = "data.db"
	var QUEUE_LABEL = "SQLiteDB"
    lazy var cachedStatement : [String:COpaquePointer] = [:]
    ...
    class func sharedInstance(path:String) -> SQLiteDB! {
        dispatch_once(&Static.token) {
            Static.instance = self.init(gid:"",path:path)
        }
        return Static.instance!
    }
...
//	}
    ...
    required init(gid:String,path:String){
...
//        let dbName:String = String.fromCString(DB_NAME)!
	...
	convenience required init(gid:String) {
		self.init(gid: gid,path: "data.db")
	}
	...
	private func closeDatabase() {
		if db != nil {
			...
			ud.synchronize()
            // Clear all the Statement
            for stmt in cachedStatement.values {
	...
	private func prepare(sql:String, params:[AnyObject]?)->COpaquePointer {
		...
		// Prepare
        if (cachedStatement[sql] != nil) {
            stmt = cachedStatement[sql]!
